...
app.get("/consulta/:id", async (request, response) => {
  try {
...
app.get('/prontuario', async (request, response) => {
  try {
...
app.get("/prontuario/:id", async (request, response) => {
  try {
    ...
    if (!prontuarios) return response.status(404).send("Pontuario não existe!")
    return response.json(prontuarios)
  }
  catch (e) {
    console.log(e)
...
app.put("/prontuario/:id", async (req, res) => {
  try {
    const { body, params } = req
    const bodyKeys = Object.keys(body)
    for (const key of bodyKeys) {
    ...
    if (error.code === "P2002") {
      res.status(404).send("Falha ao cadastrar prontuario: prontuario já cadastrado!")
    }
  }
})

app.delete("/prontuario/:id", async (req, res) => {
  const { params } = req
  try {
